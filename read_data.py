...
for j in range(1, len(sys.argv)):
    if(sys.argv[j] == "-hdrsize"):
        hdrsize = float(sys.argv[j+1])
    elif(sys.argv[j] == "-nchan"):
        nchan = float(sys.argv[j+1])
    elif(sys.argv[j] == "-datasize"):
        datasize = float(sys.argv[j+1])
    elif(sys.argv[j] == "-fil"):
	fn = sys.argv[j+1]

    elif(sys.argv[i] == "-h"):
        print("python read_data.py -hdrsize headersize -nchan numofchannels -datasize datasize -fil filterbankfilename")
with open(fn) as f:
        f.seek(0,2)
        totalbyte = f.tell()
